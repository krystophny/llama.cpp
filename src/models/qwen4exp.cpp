...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa(
        ...
        float                     kq_scale,
        int                       il) {
    // Quantized KV caches use the same Hadamard rotation as the regular
    // attention path. QSA used to assert here, making --cache-type-k/v q8_0
    // unusable even though the cache layer prepared the rotation inputs.
    // Rotate Q/K before storing and undo V rotation after attention, exactly
    // as build_attn() does for dense attention.
    if (inp->self_k_rot) {
